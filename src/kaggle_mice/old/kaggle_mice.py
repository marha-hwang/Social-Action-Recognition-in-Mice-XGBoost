...
# 역할: 하나의 실험실 데이터에 대해 F-Score를 계산합니다.
#프레임 단위 집합 연산 (Set Operations): 시간 구간(Start~Stop)을 프레임 인덱스의 set으로 변환하여, 교집합(Intersection)과 차집합(Difference)을 통해 TP, FP, FN을 계산합니다.
...
# 전체 데이터를 받아 유효성을 검사하고, 실험실(lab_id) 별로 데이터를 나누어 점수를 계산한 뒤 평균을 냅니다.
# 특정 실험실의 데이터 양이 많아 전체 점수를 지배하는 것을 방지하기 위함
...
def score(solution: pd.DataFrame, submission: pd.DataFrame, row_id_column_name: str, beta: float = 1) -> float:
    ...
    # 채점에 불필요한 row_id컬럼을 제거한다.
    # mouse_fbeta함수를 호출하여 실제 점수를 계산한다.
...
# %%
class CFG:
    train_path = "/Users/haram/Desktop/카카오부캠/코드/kaggle-mice/kaggle-mice/src/kaggle_mice/kaggle/input/MABe-mouse-behavior-detection/train.csv"
    test_path = "/Users/haram/Desktop/카카오부캠/코드/kaggle-mice/kaggle-mice/src/kaggle_mice/kaggle/input/MABe-mouse-behavior-detection/test.csv"
    train_annotation_path = "/Users/haram/Desktop/카카오부캠/코드/kaggle-mice/kaggle-mice/src/kaggle_mice/kaggle/input/MABe-mouse-behavior-detection/train_annotation"
    train_tracking_path = "/Users/haram/Desktop/카카오부캠/코드/kaggle-mice/kaggle-mice/src/kaggle_mice/kaggle/input/MABe-mouse-behavior-detection/train_tracking"
    test_tracking_path = "/Users/haram/Desktop/카카오부캠/코드/kaggle-mice/kaggle-mice/src/kaggle_mice/kaggle/input/MABe-mouse-behavior-detection/test_tracking"

    model_path = "/Users/haram/Desktop/카카오부캠/코드/kaggle-mice/kaggle-mice/src/kaggle_mice/kaggle/input/MABe-mouse-behavior-detection/social-action-recognition-in-mice-xgb-catboost"
    model_name = "xgboost"
...
#     단순 KFold가 아니라 **StratifiedGroupKFold**를 사용했습니다.
# Group: 같은 비디오(Video ID)에서 나온 프레임들이 학습셋과 검증셋에 쪼개져 들어가는 것을 방지합니다 (Data Leakage 방지). 즉, 한 비디오는 통째로 학습에 쓰이거나 통째로 검증에 쓰입니다.
# Stratified: 각 폴드마다 정답 라벨(행동 종류)의 비율이 일정하게 유지되도록 나눕니다.
    n_splits = 3
    cv = StratifiedGroupKFold(n_splits)


# verbosity=0: 학습 로그를 출력하지 않음 (조용히 실행).
# random_state=42: 실행할 때마다 같은 결과가 나오도록 시드(Seed) 고정.
# n_estimators=250: 생성할 결정 트리(Decision Tree)의 개수입니다. (250번 반복 학습)
# learning_rate=0.08: 학습률입니다. 가중치를 업데이트하는 보폭을 의미하며, 너무 크면 발산하고 너무 작으면 학습이 느립니다.
# max_depth=6: 트리의 최대 깊이입니다. 깊을수록 복잡한 패턴을 배우지만 과적합(Overfitting) 위험이 커집니다.
# min_child_weight=5: 리프 노드에 포함되어야 하는 최소 가중치 합입니다. 과적합을 막는 역할을 합니다.
# subsample=0.8: 각 트리를 만들 때 전체 데이터의 80%만 무작위로 샘플링해서 사용합니다.
# colsample_bytree=0.8: 각 트리를 만들 때 전체 컬럼(피처)의 80%만 무작위로 선택해서 사용합니다.
    model = XGBClassifier(
        verbosity=0, 
        random_state=42,
        n_estimators=250, 
...
print(f"스크립트 파일이 위치한 경로: {script_path}")
# /Users/haram/Desktop/카카오부캠/코드/kaggle-mice/kaggle-mice/src/kaggle_mice


train = pd.read_csv(CFG.train_path)

# 실제로 데이터가 존재하는 쥐의 개수를 세어 'n_mice'컬럼 추가
train['n_mice'] = 4 - train[['mouse1_strain', 'mouse2_strain', 'mouse3_strain', 'mouse4_strain']].isna().sum(axis=1)

print(train.head(5))

# MABe22_'라는 이름으로 시작하는 실험실(lab_id) 데이터를 제외하고 나머지 데이터만 추출하여 새로운 변수(train_without_mabe22)에 저장하는 코드
# 검증 데이터셋을 구성하기 위함
...
# %%
# train데이터의 body_parts_tracked를 중복제거하여 저장하기 위한 코드
# 중복 없는 신체 부위 이름들로 구성된 리스트 구성
...
# %%
# 모델 학습 시 사용하지 않을 신체 부위를 지정
# 행동인식에 결정적인 역활을 하지 않거나 노이즈가 될 수 있는 부위들
drop_body_parts =  [
...
def generate_mouse_data(dataset, traintest, traintest_directory=None, generate_single=True, generate_pair=True):
    if traintest_directory is None:
        traintest_directory = f"/Users/haram/Desktop/카카오부캠/코드/kaggle-mice/kaggle-mice/src/kaggle_mice/kaggle/input/MABe-mouse-behavior-detection/{traintest}_tracking"

    # dataset : 각 비디오의 메타데이터
    for _, row in dataset.iterrows():
        lab_id = row.lab_id
        if lab_id.startswith('MABe22') or type(row.behaviors_labeled) != str: 
            continue

        # lab_id와 video_id를 통해 프레임 데이터를 가져온다.
        video_id = row.video_id
        path = f"{traintest_directory}/{lab_id}/{video_id}.parquet"
        vid = pd.read_parquet(path)

        # 노이즈가 될 수 있는 부위들을 제거함
        if len(np.unique(vid.bodypart)) > 5:
            vid = vid.query("~ bodypart.isin(@drop_body_parts)")
        # mouse_id와 bodypart를 다중 레벨 컬럼으로 하여 시계열 형태로 재구성합니다. 이제 각 프레임마다 모든 쥐의 모든 신체 부위 좌표(x, y)를 얻을 수 있습니다
        pvid = vid.pivot(columns=['mouse_id', 'bodypart'], index='video_frame', values=['x', 'y'])
        
        del vid
        gc.collect()

        # 픽셀 단위 좌표를 센티미터 단위의 물리적 거리로 정규화합니다. 이는 다른 실험실의 스케일 차이(카메라 거리)를 보정하는 핵심 단계
        pvid = pvid.reorder_levels([1, 2, 0], axis=1).T.sort_index().T
        pvid /= row.pix_per_cm_approx


        # JSON 형태의 라벨링된 행동 정보(behaviors_labeled)를 파싱하여 (agent, target, action) 컬럼을 가진 DataFrame으로 만듭니다.
        # 이 프레임은 해당 비디오에서 어떤 쥐 쌍/단일 쥐의 어떤 행동이 라벨링되었는지 목록을 제공
        ...
        # 쥐의 상호작용으로 구분하여 데이터셋을 전처리
        # yield를 사용하여 데이터를 한번에 반환하는 것이 아닌 데이터가 필요할 때마다 반환
        # 쥐가 혼자 하는 행동처리
        if generate_single:
...
def transform_single(single_mouse, body_parts_tracked, fps):
    ...
    # single_mouse : 각 포인트마다의 x, y좌표
    # X : 모든 포인트의 좌표조합을 생성하여 두 포인트의 위치차이를 저장
    X = pd.DataFrame({
...
def cross_validate_classifier(X, label, meta, body_parts_tracked_str, section):
    ...
    for action in label.columns:
        # isna : 원래 데이터와 동일한 크기의 불리언(Boolean) Series 또는 배열이 생성됩니다. 결측값이면 True, 유효한 값이면 False가 됩
        # 결측값이 아닌 데이터를 걸러냄
        action_mask = ~ label[action].isna().values
        y_action = label[action][action_mask].values.astype(int)
        # action_mask는 bool배열, X df에서 True인 값만 추출함
        ...
        if not (y_action == 0).all():
            try:
                with warnings.catch_warnings():
                    ...
                    # X_action : 모든 부위 조합의 좌표값 차이
                    # Y_action : 특정행동인지 아닌지 여부 0또는1
...
# body_parts_tracked_list : 추적되는 포인트 리스트를 담은 리스트
for section in range(1, len(body_parts_tracked_list)):
    ...
    try:
        ...
        # 추적되는 포인트가 5개 이상이면 drop_body_parts
        if len(body_parts_tracked) > 5:
            body_parts_tracked = [b for b in body_parts_tracked if b not in drop_body_parts]

        # train은 메타데이터
        # 원하는 포인트만 subset데이터로 생성
        # 모두 같은 body_parts_tracked을 가지는 train_subset을 생성 하게 됨
        train_subset = train[train.body_parts_tracked == body_parts_tracked_str]

        # 메타데이터에서 중복되는 비디오를 제거하기 위한 코드
        temp1 = train_subset[['video_id', 'frames_per_second']]
        _fps_lookup = (
        ...
        # generate_mouse_data
        # input : 메타데이터, [train, validation]
        # output : str, track, meta, annotation 
        for switch, data, meta, label in generate_mouse_data(train_subset, 'train'):
            # switch : single or pair
            # data : video_frame, body point..., 각 포인트에 대한 좌표값이 프레임마다 존재 !!!!! 결측치 채울 포인트
            # meta : video_id, agent_id, target_id, video_frame
            # label : action..., ex) rear, attack 컬럼, 확률데이터

########################################################################################################################
            # 1. 선형 보간 (가장 일반적)
            # limit=5: 연속된 결측치가 5개 이하라면 채우고, 너무 길면(트래킹 실패 등) 놔둠
            df_interpolated = data.interpolate(method='linear', limit=5, limit_direction='both')

            # 2. 스플라인 보간 (움직임이 부드러워야 할 때)
            # order=3: 3차 스플라인
            # df_interpolated = df.interpolate(method='spline', order=3, limit=5, limit_direction='both')

            # 보간 후에도 남은 결측치(앞뒤 끝부분 등)는 제거하거나 이전 값으로 대치(ffill/bfill)
            df_final = df_interpolated.fillna(method='bfill').fillna(method='ffill')
            df_final[:] = 1
...
########################################################################################################################
            ...
            # 수동으로 메모리 할동해제를 하기 위한 코드
            del data, meta, label
        gc.collect()
    
        # 데이터 전처리 과정으로 인해 single, pair을 구분하는 듯 : transform_pair, transform_single
        # 두 가지 방법의 전처리 차이는 무엇일까?
        ...
        if len(single_mouse_list) > 0:
            ...
            if CFG.mode == 'validate':
                # temp_submission_list : 특정 액션(ex:rear)에 대하여 video_id agent_id target_id action  start_frame  stop_frame 리스트
                # temp_f1_list : 추적 포인트, 액션, 정확도
                # temp_thresholds : {rear:0.24} 무슨의미???
...
# %%
if CFG.mode == 'validate':  
    # submission_list : 특정 액션(ex:rear)에 대하여 video_id agent_id target_id action  start_frame  stop_frame 리스트
    ...
    # 학습결과로 얻은 최적값과, 계산 점수를 pkl파일로 저장
